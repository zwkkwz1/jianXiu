		    ...
		    <table style="width: 100%" class="table-hover">
		        ...
		        <tbody>
		          <tr v-for="(notArranged,$index) in con.notArrangedList">
		            ...
		            <td>
		            	<input type="text" style="min-width:135px" v-model="notArranged.driverName" @click="autocompleteShow($index)" onchange="driclick()"/>
		            	<main :id="$index" v-show="selectDri == $index" class="v-autocomplete-list">
		            		<vue-instant :suggestion-attribute="suggestionAttribute" v-model="selectDriName" @enter="autocompleteHide" @selected="selectedDri" 
		            			:suggestions="driverList" :autofocus="true" type="google" placeholder="姓名"></vue-instant>
		            	</main>
...
//				  url: '/static/controllerLeft.json',
	  ...
	  autocompleteShow(index) {
	  	...
	  	this.selectDriName = '';
//	  	var mainId = this.selectDri.toString();
//	  	setTimeout(()=>{
//	  		var child = document.getElementById(mainId).children;
//	  		var childE = child[0].children[0].children[0].children[0];
////	  		var child = document.getElementById(mainId).getElementsByTagName('input');
////	  		var childInput1 = main.getElementsByTagName('input')[2];
//	  		child[1].focus()
//	  	},10000)
//	  	var child = document.getElementById("this.selectDri").children;
	  },
	  driclick() {
	  	alert('1')
	  },
	  autocompleteHide () {
	  	this.con.notArrangedList[this.selectDri].driverName = this.selectDriName;
	  	this.selectDriName = '';
	  	this.selectDri = -1;
	  },
	  selectedDri () {
...
//				  url: this.host + '/web/getAllDrivers',
	      ...
	      .then( (response) => {
	      	...
	      	if (data.type === 1) {
	      		var driverList = data.result.driverList;
						for (var i=0;i<driverList.length;i++) {
							self.driverList.push({'name':driverList[i]});
						}
	      	}
	      }).catch( (error) => {
...
.sbx-google__submit{
	...
	height: 100% !important;
	background-color: #7b93f6 !important;
}
.sbx-google{
	width: 183px;
	height: 26px;
